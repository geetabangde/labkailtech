...
export default function OrdersDatatableV1() {
  ...
  // ✅ Helper function to get request ID (tries multiple field names)
  const getRequestId = (request) => {
    return request?.revrequestid || request?.id || request?.revid || null;
  };
  ...
  return (
    <Page title="Calibration Revision Requests">
      ...
      {/* ✅ Modal with fallback field names */}
      <ApproveRejectModal
        open={modalOpen}
        revRequestId={getRequestId(selectedRequest)}
        initialReason={selectedRequest?.reason || ""}
        initialRemark={selectedRequest?.remark || ""}
        onClose={() => {
          setModalOpen(false);
