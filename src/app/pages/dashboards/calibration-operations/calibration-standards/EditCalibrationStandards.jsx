...
export default function EditCalibration() {
  ...
  useEffect(() => {
    const fetchModes = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`/calibrationoperations/calibration-standard-byid/${id}`);
  ...
  try {
      ...
      const response = await axios.post(`/calibrationoperations/calibration-standard-update/${id}`, form);
      const result = response.data;
    ...
    <Page title="Edit Mode">
      <div className="p-6">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          >
            Back to Calibration Standard 
          </Button>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          <Input
            label="Calibration Standard  Name"
            value={mode.modeName}
