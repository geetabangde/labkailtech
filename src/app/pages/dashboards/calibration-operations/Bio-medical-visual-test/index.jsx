...
export default function OrdersDatatableV1() {
  ...
  // ✅ Fetch from API
  useEffect(() => {
    const fetchVisualTestList = async () => {
      setLoading(true);
      setError(null);

      try {
        const response = await axios.get(
          "/calibrationoperations/get-visualtest-list",
        );
        ...
        if (response.data && response.data.status) {
          ...
          // ✅ Success toast
          toast.success("Visual test list loaded successfully");
        } else {
          setError("Failed to load data from server");

          // ❌ alert removed → ✅ toast
          toast.error("Failed to load visual test list");
        }
      } catch (err) {
        console.error("Error fetching visual test list:", err);

        let message = "Failed to load visual test list";
