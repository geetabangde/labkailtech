...
// PHP: if(!in_array(393, $permissions)) → redirect
...
export default function WitnessLockList() {
  ...
  const fetchProducts = useCallback(async () => {
    try {
      ...
      console.error("Error fetching witness lock list:", err);
      setProducts([]);
    } finally {
      setLoading(false);
    }
  ...
  const [columnPinning, setColumnPinning] = useLocalStorage(
    "column-pinning-approvals-witness-lock-list-1", {}
  );
  ...
  // PHP: if(!in_array(393, $permissions)) → redirect
  if (!permissions.includes(393)) {
    return (
      <Page title="Witness Locked List">
        <div className="flex h-60 items-center justify-center rounded-xl border border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-900/20">
          <p className="text-sm font-medium text-red-600 dark:text-red-400">
            ⛔ Access Denied — Permission 393 required
          </p>
        </div>
      </Page>
    );
