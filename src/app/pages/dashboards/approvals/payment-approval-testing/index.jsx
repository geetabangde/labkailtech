...
export default function PaymentApprovalTesting() {
  ...
  // ── Fetch table data ──────────────────────────────────────────────────────
  // GET /approvals/get-non-payment-list
  // PHP: paymentstatus=2, papproval IS NULL/0, status=98
  //      date between finfromdate–fintodate, ordered by trfs.id desc
  const fetchProducts = useCallback(async () => {
    try {
      setLoading(true);
      const response = await axios.get("/approvals/get-non-payment-list");
      ...
      console.error("Error fetching payment approval list:", err);
      setProducts([]);
    } finally {
      setLoading(false);
    }
  ...
  const [columnVisibility, setColumnVisibility] = useLocalStorage(
    "column-visibility-approvals-payment-approval-testing-1", {}
  );
  const [columnPinning, setColumnPinning] = useLocalStorage(
    "column-pinning-approvals-payment-approval-testing-1", {}
  );
