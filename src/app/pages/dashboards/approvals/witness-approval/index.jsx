...
export default function WitnessApprovalList() {
  ...
  // PHP: trfs WHERE witness=1 AND (wapproval IS NULL OR wapproval=0) AND status=98
  //      INNER JOIN department, date between finfromdate–fintodate, ordered by trfs.id desc
  const fetchProducts = useCallback(async () => {
    try {
      ...
      console.error("Error fetching witness approval list:", err);
      setProducts([]);
    } finally {
      setLoading(false);
    }
  ...
  const [columnPinning, setColumnPinning] = useLocalStorage(
    "column-pinning-approvals-witness-approval-list-1", {}
  );
  ...
  if (!permissions.includes(392)) {
    return (
      <Page title="Witness Approval List">
