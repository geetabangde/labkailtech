...
export default function PriorityApprovalList() {
  ...
  // ── Fetch table data ──────────────────────────────────────────────────────
  // GET /approvals/get-priority-approval
  // PHP: priority=1, approval=0, status=98, date between finfromdate–fintodate
  const fetchProducts = useCallback(async () => {
    try {
      setLoading(true);
      const response = await axios.get("/approvals/get-priority-approval");
      ...
      console.error("Error fetching priority approval list:", err);
      setProducts([]);
    } finally {
      setLoading(false);
    }
  ...
  const [columnVisibility, setColumnVisibility] = useLocalStorage(
    "column-visibility-approvals-priority-approval-1", {}
  );
  const [columnPinning, setColumnPinning] = useLocalStorage(
    "column-pinning-approvals-priority-approval-1", {}
  );
  ...
  if (!permissions.includes(392)) {
    return (
      <Page title="Priority Approval List">
