...
export function RowActions({ row, table }) {
  ...
  // GET /actionitem/get-signatories?id=:id
  // Response: { status, trf_product_id, existing_signatories[], review_authorised_by, active_admins[] }
  ...
  // ── Save signatories ──────────────────────────────────────────────────
  // POST /actionitem/update-signatories
  const handleSave = async () => {
    try {
      ...
      const res = await axios.post("/actionitem/update-signatories", {
        id,
        signatories: newSignatories.map(Number), // array of admin ids: [1, 31]
      });
  ...
  // /actionitem/view-test-report?tid=:tid&hid=:hid
  const handleFinalReport = () => {
    navigate(`/dashboards/action-items/final-reports-unsigned/view?tid=${tid ?? id}&hid=${hid ?? ""}`);
  };
  ...
  // ── PHP logic: which buttons to show ─────────────────────────────────
  // pack_type == 0 → upload flow:
  //   report == 0  → "Upload Report"
  //   report != 0  → "Final Report"
  // pack_type != 0 → "Final Report" + "Change Signatories"
  const renderButtons = () => {
    if (pack_type === 0 || pack_type === "0") {
      ...
      return (
        <button
          ...
          className="rounded-md bg-green-600 px-3 py-1.5 text-xs font-semibold text-white shadow-sm transition hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-400"
        >
          Final Report
        </button>
      );
          ...
          <div className="relative z-10 w-full max-w-lg rounded-xl bg-white shadow-2xl dark:bg-dark-700">
            ...
            <div className="px-6 py-5">
              {sigLoading ? (
                ...
                <>
                  ...
                  {/* Fields: existing_signatories[], review_authorised_by */}
                  <div className="mb-5">
            ...
            <div className="flex justify-end gap-2 border-t border-gray-200 px-6 py-4 dark:border-dark-500">
              <button
                ...
                className="rounded-lg bg-primary-600 px-5 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-primary-400 disabled:cursor-not-allowed disabled:opacity-60"
              >
                {saving ? "Saving…" : "Save changes"}
              </button>
              <button
                onClick={() => setModalOpen(false)}
                className="rounded-lg border border-gray-300 px-5 py-2 text-sm font-semibold text-gray-600 transition hover:bg-gray-50 dark:border-dark-500 dark:text-dark-300 dark:hover:bg-dark-600"
              >
                Close
              </button>
            </div>
