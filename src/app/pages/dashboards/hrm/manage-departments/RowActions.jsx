...
const confirmMessages = {
  pending: {
    description:
      "Are you sure you want to delete this manage department? Once deleted, it cannot be restored.",
  },
  success: {
    title: "Manage department Deleted",
  },
};
...
export function RowActions({ row, table }) {
  const navigate = useNavigate(); // 👈 Hook
   const handleEdit = () => {
    ...
    navigate(`/dashboards/hrm/manage-departments/edit/${id}`);
  };
  ...
  try {
    await axios.delete(`/hrm/delete-department/${id}`);
    table.options.meta?.deleteRow(row); // remove row from UI
    setDeleteSuccess(true);
     toast.success("Manage Department deleted successfully ", {
      duration: 1000,
      icon: "🗑️",
    });
  } catch (error) {
    ...
    setDeleteError(true);
     toast.error("Failed to delete Manage Department ❌", {
      duration: 2000,
    });
  } finally {
